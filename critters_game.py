...
class Bear(Critter):
    """ Representation of a bear in our critter world. This class is a child class 
    for the bear critter. Will modify certain methods to define the bear's behaviors 
    and appearance. """

    def __init__(self, location, is_grizzly):
        super().__init__(location)
        self.is_grizzly = is_grizzly
        self.move_number = 0  # intializing move_number
    ...
    def get_move(self, neighbors):
        """ Gets the next move the critter wants to make. """
        # alternates between north and west (makes a zigzag pattern)
        i = self.move_number % 2
        self.move_number += 1
        if i == 0:  # when self.move_number is even it will move north
            return Direction.NORTH
        else:  # when its odd it will move south
    ...
    def get_color(self):
        """ Returns the color with which to display this critter. """
        # if the bear is a grizzly it will appear as brown
        if self.is_grizzly == True:
            return "brown"
        # otherwise it will be white (polar bear)
        else:
            return "snow"


class Cheetah(Critter):
    """ Representation of a cheetah in our critter world. This class is a child class 
    for the cheetah critter. Will modify certain methods to define the cheetah's behaviors 
    and appearance.  """
    ...
    def __str__(self):
        ...
        This representation is used when fighting another critter.
        """
        # return the string value of the cheetah's hunger count (integer)
        return str(self.hunger)

    def get_move(self, neighbors):
        """ Gets the next move the critter wants to make. """
        # cheetah will move three steps in one direction, randomly chosen, and then repeat
        i = self.move_number % 3
        if i == 0:  # when i is divisble by 3, a new direction with randomly chosen
            self.randmove = random.choice(
                [Direction.NORTH, Direction.SOUTH, Direction.EAST, Direction.WEST])

        cheetah_move = self.randmove
        self.move_number += 1
    ...
    def fight(self, opponent):
        """ Gets the next fight move for the critter. """
        # will pounce if it is not hungry (aka hunger count equals 0)
        if self.hunger == 0:
            return Attack.POUNCE
        # otherwise it will scratch
        else:
            return Attack.SCRATCH

    def eat(self):
        """ Returns True if the critter wants to eat, False otherwise. """
        # will continue to return true unless/until hunger count equals 0
        if self.hunger == 0:
            return False
        else:
            self.hunger -= 1
...
class Lion(Critter):
    """ Representation of a lion in our critter world. This class is a child class 
    for the lion critter. Will modify certain methods to define the lion's behaviors 
    and appearance.  """
    ...
    def get_move(self, neighbors):
        """ Gets the next move the critter wants to make. """
        # for every five steps the direction will change
        i = self.move_number
        if i <= 4:  # offsetting by 1 since python starts at 0
            self.move_number += 1
            ...
            # once i/self.move_number has reached 20 moves it must be reset
            self.move_number = 0
            return Direction.EAST
    ...
    def eat(self):
        """ Returns True if the critter wants to eat, False otherwise. """
        # if the lion has fought since the last time it has eaten then it will eat again
        if self.has_fought == True:
            self.has_fought = False  # resetting validation variable to False
            return True
        # otherwise it will not eat
        else:
            return False
...
class Torero(Critter):
    """ Representation of a Torero in our critter world. This class is a child class 
    for the Torero critter. Will modify certain methods to define the Torero's behaviors 
    and appearance.  """
    ...
    def get_move(self, neighbors):
        """ Gets the next move the critter wants to make. """
        tor_move = [Direction.NORTH, Direction.CENTER, Direction.WEST, Direction.CENTER,
                    Direction.SOUTH, Direction.CENTER, Direction.EAST, Direction.CENTER]
        i = self.move_number % 8
        self.move_number += 1

        # checking to see if there are any neighbors
        if neighbors[Direction.NORTH] == None and neighbors[Direction.WEST] and neighbors[Direction.SOUTH] and neighbors[Direction.EAST]:
            self.no_neighbors = True  # if there are not then setting variable to True

        return tor_move[i]

    def eat(self):
        """ Returns True if the critter wants to eat, False otherwise. """
        # if the Torero has no neighbors than it will eat
        if self.no_neighbors == True:
            return True
    ...
    def fight(self, opponent):
        """ Gets the next fight move for the critter. """
        characters = "!@#$%^&*()_+-=ASDFGHJKPOIUYTREWQZXCVNM,.<>?/:;\|~`"  # assigning all characters (other than numbers) to characters
        if opponent in characters:  # checking to see if the opponent is another Torero, if it is our Torero will pounce
            return Attack.POUNCE
        elif opponent == "L" or opponent == "0":  # if the opponent is a Lion or Cheetah that is not hungry
            return Attack.SCRATCH  # our Torero will scratch
        else:
            return Attack.ROAR  # if the opponent is a hungry Cheetah or Bear our Torero will roar

    def get_color(self):
        """ Returns the color with which to display this critter. """
        colors = ["deep pink", "cyan3", "DarkOrchid1", "coral1",
                  "medium spring green"]  # a list of colors we liked :)
        # randomly assigning our Toreros to one of the five colors in our colors list
